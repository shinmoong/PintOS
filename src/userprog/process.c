...
struct thread* find_child_thread(tid_t child_tid, struct list* child_list){
	struct thread *t;
	struct list_elem *e;
...
process_execute (const char *file_name) 
{
  ...
  for(i=0; i<(strlen(file_name)+1); i++){
        if(file_name[i] != '\0' && file_name[i] != ' ' && file_name[i] != '\t'){
                real_file_name[i] = file_name[i];
        }
        else{
                break;
        }
...
process_wait (tid_t child_tid) 
{
  ...
  t = find_child_thread(child_tid, &thread_current()->child_list);
  if (t == NULL) {
	  return -1;
  }
  else {
...
void argument_stack(char *file_name, void **esp){
	int i;
	char *token, *save_ptr;
	size_t size_char = strlen(file_name) + 1;
	size_t  word_align = 4 - size_char%4;
	int count = 0;
	char* parse[128];
	size_t addr[128];
	size_t argv_addr;
	...
	for(i = count - 1; i >= 0; i--){
		*esp -= (strlen(parse[i]) + 1);
		addr[i] = (size_t)(*esp);
		memcpy((char*)*esp, parse[i], strlen(parse[i]) + 1);
	}
	addr[count] = 0;
	*esp -= word_align;
	for(i = count; i >= 0; i--){
	...
	argv_addr = (size_t)*esp;
	*esp -= 4;
	**(size_t **)esp = argv_addr;
	*esp -= 4;
	**(size_t **)esp = count;
	*esp -= 4;
	*(int*)(*esp) = 0;
